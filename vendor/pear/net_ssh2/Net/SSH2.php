...
abstract class Net_SSH2
{
    ...
    public function __get($key)
    {
        if (!key_exists($key, $this->allowed_options)) {
            throw new Net_SSH2_Exception(
                Net_SSH2::getMessage(self::SSH2_OPTION_NOT_VALID, $key)
            );
        }
    ...
    protected function getMessage($code, $value = null)
    {
        ...
        if (!isset($codeMessages)) {
            $codeMessages = array(
                self::SSH2_OK                => 'No error',
                self::SSH2_ERROR             => 'Unknown error',
                self::SSH2_PACKAGE_NOT_FOUND => 'Unable to find package %s',
                self::SSH2_CLASS_NOT_FOUND   => 'Unable to load class %s',
                self::SSH2_OPTION_NOT_VALID  => 'Trying to use an undefined option "%s" for the object ' . __CLASS__,
                self::SSH2_BINARY_NOT_FOUND  => 'Unable to found the SSH2 binary command "%s"',
                self::SSH2_MODULE_NOT_FOUND  => 'Unable to found the SSH2 module "%s"',
                self::SSH2_OPTION_REQUIRED   => 'The option %s is required for the object ' . __CLASS__,
                self::SSH2_PUBLIC_KEY_UNAVAILABLE => 'Unable to read the public key: %s',
                self::SSH2_UNSUPPORTED       => 'Unsupported method',
                self::SSH2_PTY_NOT_SUPPORTED => '%s. Only key authentication is supported.'
            );
        }
